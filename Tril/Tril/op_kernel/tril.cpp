...
template<typename T> class KernelTril {  // 模板类
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, uint64_t totalSize,
        ...
        //this->tileLength = this->packNumber * this->squareSize;
    ...
    __aicore__ inline void Process() {
        for(uint64_t j=0; j<batchSize; j++){
            ...
            float onetmp = 1.0;
            int64_t length = (dimLast-oneint) - diagonal;
            for(uint64_t i=0; i<dimFirst; i++){
                if(length < dimLast && length >= 0){
                    for(uint64_t k=0; k<dimLast-length; k++){
                        Gm_y.SetValue(j*squareSize + i*dimLast + k, (T)(Gm_x.GetValue(j*squareSize + i*dimLast + k)));
                    }
                }else if(length < 0){
        ...
        Gm_y.SetValue(0, (T)(Gm_x.GetValue(0)));
    }

private:
    TPipe pipe;
    GlobalTensor<T> Gm_x, Gm_y;
    uint64_t totalSize, diagonal, squareSize, dimLast, dimFirst, batchSize;
};


template<typename T> class KernelTrilFast {  // 模板类
public:
    __aicore__ inline KernelTrilFast() {}
...
private:
    ...
    __aicore__ inline void Compute(int32_t progress, uint32_t length)
    {
        ...
        // 6 有两个0    0 1 2 3 4 5
        int64_t onelength;
        Muls(yLocal, xLocal, (T)zerof, length);
        for (int j = 0; j < length/dimLast; ++j) {     // packNumber中每个H*W
            onelength = (1 + diagonal)+((progress*tileLength/dimLast+j)%dimFirst);
            if(onelength <= dimLast && onelength > 0){
                Muls(yLocal[j*dimLast], xLocal[j*dimLast], (T)onef, onelength);
